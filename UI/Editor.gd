...
func editor_update():
	if global.CurrentSelection != null:
		...
		if global.CurrentSelection.cooldown != null:
			...
			coolbox.value = global.CurrentSelection.cooldown * 5
		else:
			$CooldownContainer.visible = false
...
func _on_button_pressed():
	apply = true
	if global.CurrentSelection.cooldown != null:
		global.CurrentSelection.timer.wait_time = coolbox.value / 5
...
func _on_cooldown_box_value_changed(value):
	global.CurrentSelection.cooldown = coolbox.value / 5
